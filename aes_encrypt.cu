...
static void packing_word(uint32_t* out, const unsigned char* in) {
	uint32_t tmp;
	for(int i = 0; i < 8; i++) {
...
static void packing_word_debug(uint32_t* out, uint32_t* in) {
	uint32_t tmp;
	for(int i = 0; i < 8; i++) {
		// out[i] 		= LE_LOAD_32_rev(in + i*16 + 0 );
		// out[i+8] 	= LE_LOAD_32_rev(in + i*16 + 4);
...
******************************************************************************/
void aes128_encrypt(unsigned char* out, unsigned char* in,	uint32_t* rkeys) {
...
void pack_nonce(uint32_t *out, uint32_t *in)
{
	uint32_t state[32]; 	// 1024-bit state (8 blocks in //)
	packing_word(state, (uint8_t*) in); 
	for (int i = 0; i < 32; ++i) in[i] = state[i];
}
